...
export default function LoginView() {
  ...
  const { register, handleSubmit, formState: {errors} } = useForm({defaultValues: initial_values})

  const handleLogin = async (formData: LoginForm) => {
    try {
      const { data } = await api.post("/auth/login", formData) // el backend y frontend tiene domino independiente
      // reset()
      //toast.success(data) // toast se usa para mostrar mensajes de alerta
      localStorage.setItem('AUTH_TOKEN', data) // almecena el token en localstorage
